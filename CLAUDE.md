...
flutter run --dart-define-from-file=config/prod.json          # production backend (config/prod.json, committed)
...
flutter run --dart-define-from-file=config/local.json      # config/local.json (gitignored, from local.example.json) with http://<mac-lan-ip>:8000
```
...
- `lib/app/`: `app.dart`, `router/app_router.dart`, `theme/` (`AppColors` + `AccentColor` per `QrType`, `AppTheme` light/dark with every component theme and `AppStatusColors` (success/warning), `AppTypography`, `app_dimens.dart`: `AppSpacing` (4-based scale), `AppRadius`, `AppLayout` (max widths, 840 side-by-side breakpoint), `AppDurations`, `AppShadows`). All colors live in `AppColors`; no `Colors.xxx` in widgets, no arbitrary sizes. Inputs keep `labelText` (floating label always shown).
- Shared UI in `lib/core/widgets/`: `LoadingButton`/`ButtonSpinner`, `SectionHeader`, `IconBadge`, `EmptyState`, `ErrorMessage`, `BrandMark`, `VisibilityToggle`. Reuse them before adding a new component.
