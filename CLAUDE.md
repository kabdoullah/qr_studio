...
- Backend (`backend/`, Python 3.9+, FastAPI): `POST /api/v1/cvs` and `POST /api/v1/cards` (multipart field `file`) return `201 { "id", "url" }`. `GET /cv/{id}` and `GET /card/{id}` serve the file inline. The file type is sniffed from its content (magic bytes), IDs are 96-bit random, and the 10 MB limit is checked on `Content-Length` before reading, then again while streaming. Storage (`app/storage.py`, `FileStore` protocol): `R2FileStore` (Cloudflare R2 through `boto3`, objects `cv/<id>` and `card/<id>`, filename and content type kept on the object, no database) when the four `R2_*` env vars are set, otherwise `LocalFileStore` (disk + SQLite, for dev and tests). On Render (`RENDER` set), `Settings.from_env()` refuses to start without R2, because the disk is ephemeral. The public URL defaults to `RENDER_EXTERNAL_URL`. Uploads are rate-limited in memory (per IP and globally, returning 429). `create_app(settings, store=None)` is a factory (no module-level app), and tests inject `tmp_path` or a fake S3 client. Deployment: `render.yaml` at the repo root (free web service, `rootDir: backend`, Python 3.12). There is no auth or file expiry yet.
