...
QR Studio: a Flutter app (Android, plus a web PWA for iOS users) that creates QR codes from four content types only: **business card** (vCard from a form, *or* a link to an uploaded image of the card), **CV as a PDF file**, **free text**, and **social media page** (a public page listing several social links, served by the backend). V1 aims to be small, polished, and fast, not feature-rich. Toolchain: Flutter 3.44 / Dart 3.12 (`sdk: ^3.12.0`), lints from `flutter_lints`.

State as of 2026-09-24: steps 2–10 are done, step 11 (audit) is partly done, and the business-card image mode, the app icon/splash, and the web PWA were added after it, then the social media page type. The user has tested the app on Android, and the app label is now "QR Studio". A minimal FastAPI backend lives in `backend/` (see below). It is deployed on Render at `https://qr-studio-api-q71x.onrender.com`, with Neon Postgres. **Shared business cards:** the user chose a public directory, so any user sees every published card. Publishing is an explicit button with a confirmation dialog that says the card is public and can't be deleted from the app. Generating a QR never publishes anything. `SavedCardsView` (`/qr/cards`) lists and searches cards (`savedCardsProvider`, autoDispose, 350 ms debounce, stale responses ignored, `retry: null`). Tapping a card calls `loadBusinessCard`, which bumps `businessCardRevision`, the `Form` key that rebuilds the fields with the loaded values. `CardPublishViewModel` handles publishing. `businessCardDirectoryProvider` is `null` without `QR_STUDIO_API_URL` (in `core/constants/api_config.dart`), which hides both buttons and redirects `/qr/cards` to `/`. **Social media page** (`QrType.socialPage`, last on the home screen, inspired by MyQRCode's concept, not its design): the user chose an immutable page (editing means publishing a new page and QR), no photo or theme, visible only through its link (not listed anywhere). Unlike the other types, **"Générer" publishes**: the view shows `SocialPagePublishDialog` first (only when the form is valid), then `generateQr()` calls `SocialPageService.publish` and encodes the returned `/s/{id}` URL. State: `QrContentState.socialPage` (`SocialPageData { title, bio, links: [SocialLink { id, network, value }] }`, raw input kept for "Modifier") and `socialPagePublish` (`PublishState`). `SocialNetwork` (enum, `models/social_network.dart`) owns URL normalization (`normalize`, also used by the vCard) and validation (`toUrl`: https, network domain, same rules as the server). `socialPageServiceProvider` is `null` without `QR_STUDIO_API_URL` (honest "not available yet" message). `QrCodeData.isOnlineLink` shows and shares the link for files and pages. Run native builds one at a time: an earlier parallel build filled the disk. Flow state lives in two non-auto-dispose providers: `qrGeneratorViewModelProvider` (selected `QrType`) and `qrContentViewModelProvider` (`QrContentState`: business card data + `BusinessCardMode` (`details`/`image`), text, one `FileState` per `SharedFileKind` (`cv`, `businessCardImage`, stored in the `cv`/`cardImage` fields), `showErrorsFor` (the set of types whose generation failed, so errors never leak onto another type's form), and the last `QrCodeData` result). Validation rules are static methods on `QrContentState`, shared by the form validators and the `is…Valid` getters. Payload size is checked in bytes by `QrService.fitsInQrCode` (the default error correction level, M, holds at most 2331 bytes, and emojis take 4 bytes each). Live preview: `livePreviewProvider` (a derived `Provider<LivePreview?>`, `null` for file-backed content) feeds `QrLivePreview`. `QrContentView` places it under the form on phones and beside it at 840 dp and wider. Forms don't render the preview themselves. `generateQr()` is async because file-backed content is uploaded first. On success the view pushes `/qr/result`. "Modifier" pops back to the form, and "Créer un nouveau QR Code" does `go('/')` then `startOver()`, which resets both view models. Result actions live in `QrResultViewModel` (one action at a time, returns a user message for a SnackBar). `QrExportService` renders a 1024 px PNG through `QrStyle.toDecoration(forExport: true)`, the same rendering as the preview plus background and quiet zone. `QrShareService` shares through `share_plus`, writing to `systemTemp/qr_studio_share/`, which it clears *before* each share because the receiving app may read the file after the share sheet returns. It saves through `FilePicker.saveFile`, the system "Save as" dialog. Router redirects send `/qr/content` (no type selected) and `/qr/result` (no result) back to `/`. `fileStorageServiceProvider` returns `HttpFileStorageService` when the app is started with `--dart-define=QR_STUDIO_API_URL=…`, and otherwise `BackendRequiredFileStorageService`, which always throws `FileStorageUnavailableException`, so file flows stop with an honest "not available yet" message. Test fakes live in `test/helpers/`. App services are faked by overriding their providers (`fake_services.dart`). The native `file_picker` platform is faked with `FilePickerPlatform.instance = FakeFilePickerPlatform()`, and `share_plus` by mocking its `dev.fluttercommunity.plus/share` method channel. Home cards and result buttons can sit below the fold on the test screen, so call `tester.ensureVisible` before `tap`. Much of what follows is the target design, so check what is actually present before you assume a file or dependency exists.
...
- `FilePickerService.pickPdf()` / `pickImage()` return a `SharedFile { name, size, localPath?, bytes?, remoteUrl? }` without loading the content into memory (except on web, where `bytes` is the only option). `pickImage` asks iOS for a compatible (JPEG) representation.
...
- Backend (`backend/`, Python 3.9+, FastAPI): `POST /api/v1/cvs` and `POST /api/v1/cards` (multipart field `file`) return `201 { "id", "url" }`. Business cards (`app/cards.py`, one `CardStore` SQL for SQLite and Postgres, `business_cards` table deduplicated by content hash): `POST /api/v1/business-cards` (JSON, snake_case fields, 200 chars max, first and last name required) and `GET /api/v1/business-cards?q=&limit=`. Social pages (`app/social_pages.py`, `social_pages` table, deduplicated by content hash): `POST /api/v1/social-pages` returns `201 { "id", "url" }` and `GET /s/{id}` renders a self-contained HTML page (escaped values, no JavaScript, strict CSP, `noindex`). `app/database.py` holds the SQL access shared by cards and pages. `GET /cv/{id}` and `GET /card/{id}` serve the file inline. The file type is sniffed from its content (magic bytes), IDs are 96-bit random, and the 10 MB limit is checked on `Content-Length` before reading, then again while streaming. Storage (`app/storage.py`, `FileStore` protocol): `PostgresFileStore` (Neon, files stored as `bytea` in the auto-created `files` table, one connection per operation with `prepare_threshold=None` for Neon's pooler) when `DATABASE_URL` is set, otherwise `LocalFileStore` (disk + SQLite, for dev and tests). On Render (`RENDER` set), `Settings.from_env()` refuses to start without `DATABASE_URL`, because the disk is ephemeral. Total storage is capped (`QR_STUDIO_MAX_STORAGE_MB`, default 400, under Neon's 0.5 GB free tier), returning 507 when full. The public URL defaults to `RENDER_EXTERNAL_URL`. Uploads are rate-limited in memory (per IP and globally, returning 429). `create_app(settings, store=None)` is a factory (no module-level app). Postgres tests run against a real embedded server (`pgserver`, fixtures in `tests/conftest.py`). Deployment: `render.yaml` at the repo root (free web service, Frankfurt region to match Neon, `rootDir: backend`, Python 3.12). There is no auth or file expiry yet.
- Web (PWA, for iOS users who install it via Safari › "Sur l'écran d'accueil"): a browser gives no file path, so on web `SharedFile.bytes` holds the content and `HttpFileStorageService.upload` sends it with `MultipartFile.fromBytes` (mobile streams from `localPath`). `QrShareService` branches on `kIsWeb`: share from memory (Web Share API), and `savePng` always returns `true` because the browser downloads directly. Any `dart:io` use must stay behind such a branch. File picking on web passes `FilePickerWebOptions(cancelUploadOnWindowBlur: false)` (`services/picker_web_options_web.dart`, loaded by a conditional import on `dart.library.js_interop`; `picker_web_options_stub.dart` elsewhere, because `file_picker_web` doesn't compile on mobile). With the default, iOS Safari lost the chosen file: the page regains focus when the picker closes, and the file often arrives more than 500 ms later. Only the browser's `cancel` event (iOS 16.4+) now reports a cancellation. The backend's `QR_STUDIO_CORS_ORIGINS` must list the exact PWA origin (set in the Render dashboard). For local web runs, pin the port (`flutter run -d chrome --web-port 8080`) and allow `http://localhost:8080`. `web/index.html` and `web/manifest.json` are hand-tuned for iOS install. `render.yaml` defines two services: `qr-studio-api` (backend) and `qr-studio-web` (static site that clones Flutter `$FLUTTER_VERSION` at build time; keep it in sync with the project's Flutter version). The backend only accepts browser calls from `QR_STUDIO_CORS_ORIGINS` (comma-separated, empty by default).
...
- Out of scope for V1 (don't add): auth or accounts, cloud history, analytics, payments, scan statistics, dynamic QR codes (including editing a published social page), or a PDF generator. The backend stays minimal (file upload and serving, shared cards, social pages). Don't start V2 work without an explicit request.
